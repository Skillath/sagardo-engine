...
namespace SagardoEngine
{
    ...
    public:
        ...
        explicit Model(const std::filesystem::path& path, const bool gamma = false) : _gammaCorrection(gamma)
        {
            LoadModel(path);
        }
    ...
    private:
        ...
        void LoadModel(const std::filesystem::path& path)
        {
            ...
            {
                _directory = path.root_directory();
            }
            else
            {
                _directory = path;
            ...
            const std::string& typeName)
        {
            ...
            for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
            {
                ...
                {
                    if (std::strcmp(_texturesLoaded[j].Path.c_str(), str.C_Str()) == 0)
                    {
                ...
                if (!skip)
                {
                    ...
                    Texture texture;
                    texture.Id = GlTextureLoader::LoadTextureFromFile(_directory/str.C_Str(), false).Id;
