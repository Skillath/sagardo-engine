...
namespace SagardoEngine
{
    ...
    int Application::Run() const
    {
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            { // every second
                const double fps = static_cast<double>(nbFrames) / (currentTime - lastTime);
                std::string message = _settings.Name + "- FPS: " + std::to_string(static_cast<int>(fps));
            ...
            const auto deltaTime = static_cast<float>(TimeProvider::GetDeltaTime());
